...
class saveToExcel:
    ...
    def saveCertificateInfo(self, crawlCerts_context):
        self.sheet.cell(self.Sheet_line, 1).value = '子域名'
        self.sheet.cell(self.Sheet_line, 2).value = '证书信任域名'
        self.Sheet_line += 1
        for each_result in crawlCerts_context['result']:
            for trust_domain in each_result['trust_domain']:
                self.sheet.cell(self.Sheet_line, 1).value = each_result['subdomain']
                self.sheet.cell(self.Sheet_line, 2).value = trust_domain
                self.Sheet_line += 1
        self.excel.save(self.excelSavePath)

    def saveSubdomainInfo(self, subdomainInfo_context):
        self.sheet.cell(self.Sheet_line, 1).value = '子域名'
        self.sheet.cell(self.Sheet_line, 2).value = 'A记录IP'
        self.sheet.cell(self.Sheet_line, 3).value = 'CDN'
        self.sheet.cell(self.Sheet_line, 4).value = '来源'
        self.Sheet_line += 1

        for each_result in subdomainInfo_context['result']:
            subdomain, ip, cdn, refer = each_result['subdomain'], each_result['ip'], str(each_result['cdn']), each_result['refer']
            self.sheet.cell(self.Sheet_line, 1).value = subdomain
            self.sheet.cell(self.Sheet_line, 2).value = ip
            self.sheet.cell(self.Sheet_line, 3).value = cdn
            self.sheet.cell(self.Sheet_line, 4).value = refer
            self.Sheet_line += 1
        self.excel.save(self.excelSavePath)


    def saveCsubnetInfo(self, cipList_context):
        self.sheet.cell(self.Sheet_line, 1).value = 'C段'
        self.sheet.cell(self.Sheet_line, 2).value = '数量'
        self.sheet.cell(self.Sheet_line, 3).value = 'IP'
        self.Sheet_line += 1
        for each_result in cipList_context['result']:
            for ip in list(set(each_result['c_ips'])):
                csubnet, num = each_result['c_ip'], each_result['ip_num']
                self.sheet.cell(self.Sheet_line, 1).value = csubnet
                self.sheet.cell(self.Sheet_line, 2).value = num
                self.sheet.cell(self.Sheet_line, 3).value = ip
                self.Sheet_line += 1
        self.excel.save(self.excelSavePath)

    def saveCsubnetCertInfo(self, domain, crawlCsubnetCerts_context):
        self.sheet.cell(self.Sheet_line, 1).value = 'C段'
        self.sheet.cell(self.Sheet_line, 2).value = 'IP'
        self.sheet.cell(self.Sheet_line, 3).value = '证书域名'
        self.sheet.cell(self.Sheet_line, 4).value = '是目标'
        self.Sheet_line += 1
        for each_result in crawlCsubnetCerts_context['result']:
            ip = each_result['subdomain']
            csubnet = f"{ip.rsplit('.', 1)[0]}.0"
            # logger_print_content(f"ip: {ip}, csubnet:{csubnet}")
            for trust_domain in each_result['trust_domain']:
                # logger_print_content(f"each_trust_domain: {each_trust_domain}")
                trust_domain = trust_domain.strip("*.")
                if domain in trust_domain:
                    is_target = 1
                else:
                    is_target = 0
                self.sheet.cell(self.Sheet_line, 1).value = csubnet
                self.sheet.cell(self.Sheet_line, 2).value = ip
                self.sheet.cell(self.Sheet_line, 3).value = trust_domain
                self.sheet.cell(self.Sheet_line, 4).value = is_target
                self.Sheet_line += 1
        self.excel.save(self.excelSavePath)

    def saveWebspaceInfo(self, WebSpaceSearchEngine_context):
        ...
        self.sheet.cell(self.Sheet_line, 12).value = '公司名'
        self.sheet.cell(self.Sheet_line, 13).value = 'isp'
        self.sheet.cell(self.Sheet_line, 14).value = '收录时间'
        self.Sheet_line += 1

        for each_result in WebSpaceSearchEngine_context['result']:
            webSpaceName, host, title, ip, subdomain, port, server, protocol, address, query_str, robots, company, isp, updated_at = \
            each_result['webSpace_name'], each_result['host'], each_result['title'], each_result['ip'], each_result[
                'subdomain'], each_result['port'], each_result['server'], each_result['protocol'], each_result[
                'address'], each_result['query_str'], each_result['robots'], each_result['company'], each_result['isp'], \
            each_result['updated_at']
            ...
            self.sheet.cell(self.Sheet_line, 11).value = robots
            self.sheet.cell(self.Sheet_line, 12).value = company
            self.sheet.cell(self.Sheet_line, 13).value = isp
            self.sheet.cell(self.Sheet_line, 14).value = updated_at
            self.Sheet_line += 1
        self.excel.save(self.excelSavePath)

    def saveUrlTarget(self, webServer_list):
        self.sheet.cell(self.Sheet_line, 1).value = 'URL'
        self.Sheet_line += 1
        for url in webServer_list:
            self.sheet.cell(self.Sheet_line, 1).value = url
            self.Sheet_line += 1
        self.excel.save(self.excelSavePath)


    def saveObserverWard(self, observerWard_result):
        self.sheet.cell(self.Sheet_line, 1).value = 'URL'
        self.sheet.cell(self.Sheet_line, 2).value = '指纹'
        self.sheet.cell(self.Sheet_line, 3).value = '标题'
        self.sheet.cell(self.Sheet_line, 4).value = '状态码'
        self.Sheet_line += 1
        for each_result in observerWard_result:
            url, status_code, title, fingerprint_name = each_result
            self.sheet.cell(self.Sheet_line, 1).value = url
            self.sheet.cell(self.Sheet_line, 2).value = fingerprint_name
            self.sheet.cell(self.Sheet_line, 3).value = title
            self.sheet.cell(self.Sheet_line, 4).value = status_code
            self.Sheet_line += 1
        self.excel.save(self.excelSavePath)

    def saveMasNmap(self, all_masNmap_results):
        self.sheet.cell(self.Sheet_line, 1).value = '主机'
        self.sheet.cell(self.Sheet_line, 2).value = '端口'
        self.sheet.cell(self.Sheet_line, 3).value = '服务'
        self.Sheet_line += 1
        for each_result in all_masNmap_results:
            host, port, service_name = each_result
            self.sheet.cell(self.Sheet_line, 1).value = host
            self.sheet.cell(self.Sheet_line, 2).value = port
            self.sheet.cell(self.Sheet_line, 3).value = service_name
            self.Sheet_line += 1
        self.excel.save(self.excelSavePath)
    ...
    # 保存证书结果
    def saveCert(self, crawlCerts_context):
        self.sheet.cell(self.Sheet_line, 1).value = '子域名'
        self.sheet.cell(self.Sheet_line, 2).value = '证书信任域名'
        self.Sheet_line += 1
